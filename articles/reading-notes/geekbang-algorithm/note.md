...
根据摊还分析法，每次需要扩容的 push 的复杂度可以均摊到普通的不需要扩容时 push，因此其均摊时间复杂度是 O(1)
...
#### 4.浏览器的前进后退
使用两个栈 A 和 B，浏览过程中把浏览的界面按顺序压入栈 A 中。

点击返回的按钮时，让栈 A 栈顶出栈，出栈的数据压入栈 B 中。

点击前进按钮时， 让栈 B 栈顶出栈，出栈的数据压入栈 A。

栈 A 为空说明无法后退， 栈 B 为空说明无法前进。
...
- **编写递归代码的关键是，只要遇到递归，我们就把它抽象成一盒递推公式，不用想一层层的代用 关系，不要试图用人脑分解递归的每个步骤**
