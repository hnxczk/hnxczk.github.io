...
## 说一下什么是悬垂指针？什么是野指针?
- 悬垂指针：
指向曾经存放对象的内存，但是该对象已经不存在了；delete操作完成后的指针就是悬垂指针，此时需要将指针置为0变为零值指针；
...
```
    ...
    a = [NSMutableString stringWithFormat:@"Jim"];
    NSLog(@"\n 定义以后：%@-a指向的堆中地址：%p；a在栈中的指针地址：%p", a, a, &a);
    foo();
...
```
2018-04-24 18:13:22.720095+0800 run[80082:26270610] 
 定义以前：Tom-a指向的堆中地址：0x60000024fb40；a在栈中的指针地址：0x7fff5a51bc58
2018-04-24 18:13:22.720253+0800 run[80082:26270610] 
 定义以后：Jim-a指向的堆中地址：0x60000024fc30；a在栈中的指针地址：0x7fff5a51bc58
2018-04-24 18:13:22.720387+0800 run[80082:26270610] 
 block内部：Tom-a指向的堆中地址：0x60000024fb40；a在栈中的指针地址：0x60000024ff80
2018-04-24 18:13:22.720517+0800 run[80082:26270610] 
 调用以后：Jim-a指向的堆中地址：0x60000024fc30；a在栈中的指针地址：0x7fff5a51bc58
```
block 内输出 Tom, 但是**当 a 被 block 捕获之后 block 外部的 a 的地址已经发生了变化**
