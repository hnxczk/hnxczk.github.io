...
(在查找 `FAST_DATA_MASK` 定义的时候，你会发现它的周围有很多类似的定义，根据宏 `__LP64__` 来判断到底编译哪个。关于 `__LP64__` 的更多信息可以看[这里](https://stackoverflow.com/questions/6721037/where-is-lp64-defined-for-default-builds-of-c-applications-on-osx-10-6)。你可以通过在终端中通过 `cpp -dM /dev/null` 命令来查看本机预处理程序宏定义的列表。我电脑是`#define _LP64 1`。 估计你的也是这样的，毕竟都是64位的机器。。)
