...
如果在上面 `cls->setData(rw);` 执行之后打断点的话你会发现 `rw` 的方法列表中是空的，需要通过 `methodizeClass` 函数来将 `ro` 里面的 `baseMethods`、`baseProperties`、`baseProtocols` 分别添加到 `rw` 中的 `methods`、`properties`、`protocols` 之中。
