...
bucket_t * cache_t::find(cache_key_t k, id receiver)
{
    ...
    // 下面是通过线性探测的方法来查找,所以该方法会返回找到的 key 对应的 bucket_t，找不到就返回数组中最后为空的位置。
    mask_t i = begin;
    do {
...
static void cache_fill_nolock(Class cls, SEL sel, IMP imp, id receiver)
{
    ...
    // 根据上面 find 函数的分析可以看出 bucket 里的内容是可能为空的。也就是找不到的情况下返回一个空白的 bucket，这时候需要添加 _occupied 数量
...
通过对比新旧两版的 `objc_class` 的实现，可以看出来 `isa`、`superclass`、`cache` 这些是相同的。
