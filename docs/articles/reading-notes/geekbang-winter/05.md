...
这里需要注意的是 undefined 是一个值，也就是说在使用 a 的时候，a 已经被声明了。这就很奇怪了，明明 a 的声明在这一行下面，为什么这个时候就已经被声明了呢？
...
> 注意：
> 1. 上面提到的都是函数的执行上下文，全局执行上下文是没有 参数、arguments、作用域链 这几个概念的。
> 
> 2. 作用域链 和 this 的指向我们后面再详细了解
